...
import Test.QuickCheck (stdArgs, Arbitrary, arbitrary, Gen)
import Servant.QuickCheck
import Servant.QuickCheck.Internal.ErrorTypes (PredicateFailure(..))


import Server (DocumentedAPI, server, jwtSettings, cookieSettings)
...
instance Arbitrary Auth.UserLoginData where
    arbitrary = Auth.UserLoginData 
                <$> arbitrary 
                <*> arbitrary
  
instance Arbitrary Auth.UserRegisterData where
    arbitrary = Auth.UserRegisterData
                <$> arbitrary
                <*> arbitrary
                <*> arbitrary
...
betterNot500 = RequestPredicate $ \request manager -> do
                  ...
                  when (responseStatus response == status500) $ 
                      throw $ PredicateFailure "betterNot500" (Just request) response
                  return []
...
followsBestPractices = describe "API" $ do
    ...
    it "follows best practices" $ do
        ...
        withServantServerAndContext (Proxy :: Proxy (DocumentedAPI Auths)) 
                                    context 
                                    (testServer jwk) $ 
                                    \burl ->
            serverSatisfies (Proxy :: Proxy (DocumentedAPI Auths)) burl stdArgs
